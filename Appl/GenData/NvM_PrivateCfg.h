...
typedef P2VAR(uint8, AUTOMATIC, NVM_PRIVATE_DATA) NvM_RamDataIntegrityAddressType;
...
typedef struct
{
    ...
    NvM_RamDataIntegrityAddressType RamBlockDataIntegrityAddr_t; /* data integrity value for consistency checks */
#endif
    NvM_RamDataIntegrityAddressType CRCCompMechanismCrcAddr_t; /* CRC for write avoiding mechanism */
    ...
    NvM_DataIntegrityType           DataIntegritySettings :3;
} NvM_BlockDescriptorType;

/* CSM related part: include, defines (configuration dependent) and types. */

#define NVM_NR_OF_CSM_CRYPTO_JOBS 0u 
#define NVM_NR_OF_CSM_MAC_JOBS    0u 
...
} NvM_CsmCryptoJobType; /*!< Type to store all relevant values of a NvM CSM crypto reference. */

typedef struct
{
  NvM_CsmJobIdType GenerateJobId; /*!< MAC generation job identfier forwarded to the CSM. */
  NvM_CsmJobIdType VerifyJobId; /*!< MAC verify job identfier forwarded to the CSM. */
} NvM_CsmMacJobType; /*!< Type to store all relevant values of a NvM CSM MAC reference. */
