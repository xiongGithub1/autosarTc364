...
LOCAL_INLINE void Fee_lGcHardenQsSector(void);

LOCAL_INLINE boolean Fee_lIsQsWriteReadAllowedbyGC(void);
LOCAL_INLINE Fls_17_Dmu_AddressType Fee_lGcHardenReqChk(void);
...
static Std_ReturnType Fee_lGetCycleSafetyCheck\
          (const uint32 * const CountPtr);
#endif
#endif

#if (FEE_DATA_BLOCK_SUPPORTED != FEE_DOUBLE_SECTOR_DATA_ONLY)
static Std_ReturnType Fee_lQsBlkInfoSafetyCheck \
