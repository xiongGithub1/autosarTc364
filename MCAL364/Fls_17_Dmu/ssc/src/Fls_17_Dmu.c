...
#if ( FLS_17_DMU_AR_RELEASE_REVISION_VERSION != MCAL_AR_RELEASE_REVISION_VERSION)
...
#if (FLS_17_DMU_SW_MINOR_VERSION != 10U)
  ...
  #endif
  {
    ...
    if(Fls_lChkOperError() == 0U)
    {
      ...
      if(Fls_lHWBusyCheck() == 0U)
      {
        ...
        StatePtr->FlsEraseCmdTimeoutTicks = FLS_17_DMU_ERASE_TIME/TimeOutResolution;
        #endif
    ...
    if(Fls_lChkOperError() == 0U)
    {
      ...
      if(Fls_lHWBusyCheck() == 0U)
      {
        ...
        #endif /* ERASE VERIFICATION */
        {
          ...
          TimeOutResolution = Mcal_DelayTickResolution();
          StatePtr->FlsWriteCmdTimeoutTicks = FLS_17_DMU_WRITE_TIME/TimeOutResolution;
          #endif
...
[/cover] */
{
  ...
  else if(DMU_HF_ERRSR.B.EVER != 0U)
  {
      ...
      #endif
      #if (FLS_17_DMU_RUNTIME_ERROR_DETECT == STD_ON)
      (void)Det_ReportRuntimeError( (uint16)FLS_17_DMU_MODULE_ID,FLS_17_DMU_INSTANCE_ID,
                          FLS_17_DMU_SID_MAIN, FLS_17_DMU_E_ERASE_FAILED);
      #endif
      #endif
...
static void Fls_lMainCompare(void)
{
  ...
  if(ErrorFlag!= 0U)
  {
    ...
    Report FLS_17_DMU_E_COMPARE_FAILED (Runtime/Transient fault) error according to the Autosar version
    [/cover] */
    #if(MCAL_AR_VERSION == MCAL_AR_440)
    (void)Det_ReportTransientFault( (uint16)FLS_17_DMU_MODULE_ID,
                              FLS_17_DMU_INSTANCE_ID,
...
static void Fls_lMainWriteJobStart(const uint32 PhysicalAddress)
{
  ...
  if(SeqProtErr != 0U)
  {
    ...
    Report write error (Runtime/transient fault) according to the Autosar version
    [/cover] */
    #if(MCAL_AR_VERSION == MCAL_AR_440)
    (void)Det_ReportTransientFault( (uint16)FLS_17_DMU_MODULE_ID,
                            FLS_17_DMU_INSTANCE_ID,
    ...
    const uint8 ServiceID)
{
  ...
  /*MISRA2012_RULE_11_3_JUSTIFICATION: Pointer cast is required to match
     the object types as the address of the register is being passed.*/
  FLS_17_DMU_LIB_RUNTIME_WRITECPUENDINITPROTREG((volatile uint32 *)&DMU_HF_ECCC.U,\
        BitChange);
