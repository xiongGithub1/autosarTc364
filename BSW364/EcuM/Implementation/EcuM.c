...
#if (  (ECUM_SW_MAJOR_VERSION != (0x12u)) \
...
/* vendor specific version information is BCD coded */
#if (  (ECUM_CBK_SW_MAJOR_VERSION != (0x12u)) \
...
# define ECUM_SLEEP_RESOURCE                                  EcuM_InternalGetPartitionIdx()
#else
...
# if (ECUM_ISDEF_WAKEUPSOURCEOFSLEEPMODELIST == STD_OFF)
    ...
    if ((EcuM_EnabledWakeups & (~EcuM_GetWakeupSourceOfSleepModeList(localShutdownMode))) != 0u)
    {
      EcuM_DisableWakeupSources(EcuM_EnabledWakeups & (~EcuM_GetWakeupSourceOfSleepModeList(localShutdownMode)));
    }
# endif
...
#  if(ECUM_RESOURCEOFPARTITIONDATA == STD_ON)
  EcuM_SizeOfPartitionDataType applicationIdx = EcuM_InternalGetPartitionIdx();
  /* #60 Block the scheduler to avoid interruption of the sleep phase by other tasks. */
  if ((applicationIdx < EcuM_GetSizeOfPartitionData() )
    /* In case of a non-EcuM partition, Resource handling is not existent */
#   if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
    && (EcuM_IsEcuMPartitionOfPartitionData(applicationIdx))
#   endif
    )
  {
    (void)GetResource(EcuM_GetResourceOfPartitionData(applicationIdx));
#  endif
...
#  if(ECUM_RESOURCEOFPARTITIONDATA == STD_ON)
    /* #61 Unlock the scheduler to continue task scheduling. */
    (void)ReleaseResource(EcuM_GetResourceOfPartitionData(applicationIdx));
  }
#  endif
...
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_InternalKillAllRequests(uint8 mask)
{
  ...
  EcuM_SetKillAllInProgress((EcuM_KillAllInProgressType)(EcuM_GetKillAllInProgress() | mask)); /* SBSW_ECUM_CSL_VAR_ACCESS */
...
# if(ECUM_ECUMPARTITIONOFPARTITIONDATA == STD_ON)
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
      && (applicationIdx < EcuM_GetSizeOfPartitionData())
      && (EcuM_IsEcuMPartitionOfPartitionData(applicationIdx))
#  endif
# endif
      )
...
#  if(ECUM_ISDEF_ECUMPARTITIONOFPARTITIONDATA == STD_OFF)
      if(EcuM_IsEcuMPartitionOfPartitionData(applicationIdx))
#  endif
      {
        /* #32 Initialization of the schedule Manager. */
#  if(ECUM_SCHM_START_INITIALIZATION == STD_ON)
...
#   if(ECUM_ISDEF_BSWMPARTITIONOFPARTITIONDATA == STD_OFF)
      if(EcuM_IsBswMPartitionOfPartitionData(applicationIdx))
#   endif
      {
        /* #33 Initialization of the BswM. */
        BswM_Init( ECUM_BSWM_CONFIG_POINTER); /* SBSW_ECUM_FUNCTIONCALL_POINTER */
