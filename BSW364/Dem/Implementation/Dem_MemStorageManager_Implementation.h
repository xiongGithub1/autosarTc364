...
/* ****************************************************************************
 * Dem_MemStorageManager_TestSRecUpdateRequired
 *****************************************************************************/
/*!
  ...
  )
{
  ...
  if (
# if (DEM_FEATURE_NEED_SREC_OEMTRIGGER == STD_ON)
    (Dem_MemStorageManager_TestOemSRecTrigger(SnapshotEntryIter->EventId, SnapshotEntryIter->MemoryEntryId, CfgSRecIndex, UpdateFlags)) ||
# endif
    (Dem_Cfg_SRecTrigger(CfgSRecIndex) == (UpdateFlags & Dem_Cfg_SRecTrigger(CfgSRecIndex))))
  {
    if (Dem_Memory_UserDefinedMemory_IsSRecNumerationCalculatedFiFo(RecNumType))
    {
      lUpdateNeeded = (Dem_SnapshotEntry_IteratorTestSnapshotRecordFifoStorageRequired(SnapshotEntryIter) == TRUE);              /* SBSW_DEM_POINTER_FORWARD_STACK */ /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
    }
    else
    {
      lUpdateNeeded = ((Dem_Cfg_SRecUpdate(CfgSRecIndex) == TRUE)
                      || (Dem_SnapshotEntry_IteratorIsSnapshotRecordStored(SnapshotEntryIter) == FALSE));                        /* SBSW_DEM_POINTER_FORWARD_STACK */ /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
    }
  }
  ...
  )
{
  ...
  Dem_Cfg_SRecNumerationType lRecNumType = Dem_Memory_GetSRecNumerationType(Dem_Cfg_EventDestination(EventId));
       ...
       Dem_SnapshotEntry_IteratorNext(&lSnapshotEntryIter))                                                                      /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
    ...
    if (Dem_MemStorageManager_TestSRecUpdateRequired(lCfgSRecIndex, lRecNumType, UpdateFlags, &lSnapshotEntryIter) == TRUE)      /* SBSW_DEM_POINTER_FORWARD_STACK */
    {
      Dem_SnapshotEntry_IteratorUpdateSnapshotEntry(&lSnapshotEntryIter);                                                        /* SBSW_DEM_POINTER_FORWARD_STACK */

# if (DEM_CFG_SUPPORT_TYPE2_COMBINATION == STD_ON)
      if (Dem_Memory_UserDefinedMemory_IsSRecNumerationConfigured(lRecNumType))
      {
...
# endif
      ...
      if (Dem_Memory_UserDefinedMemory_IsSRecNumerationCalculated(lRecNumType))
      {
 ...
 */
DEM_LOCAL_INLINE FUNC(uint8, DEM_CODE)
Dem_MemStorageManager_TestObdIIFreezeFrameUpdates(                                                                               /* PRQA S 3219 */ /* MD_DEM_14.1_LIB */
  CONSTP2CONST(Dem_DTC_ContextType, AUTOMATIC, AUTOMATIC) DTCContext,
  CONST(uint8, AUTOMATIC)  FreezeFrameIndex
  )
{
  ...
  if (FreezeFrameIndex != Dem_Cfg_ObdFFSlotCountInVariant())
  {
...
# endif
  ...
  DEM_IGNORE_UNUSED_VARIABLE(lMemoryEntry)                                                                                       /* PRQA S 3112 */ /* MD_DEM_14.2 */
  return lBlockModification;
}                                                                                                                                /* PRQA S 6050 */ /* MD_MSR_STCAL */
...
# endif
    ...
    lFreezeFrameSlot = Dem_Mem_FindOBDFreezeFrameSlot(DTCContext, lDisplacePendingFreezeFrame);                                /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
...
# if (DEM_CFG_SUPPORT_CONFIRMED_STORAGE_FREEZE_FRAME == STD_ON) && (DEM_CFG_SUPPORT_CONFIRMED_VISIBLE_FREEZE_FRAME == STD_ON)
    {
      ...
      lUpdatedEventId = Dem_FaultMemory_MemObdFreezeFrame_UpdateOBDIIFreezeFrame(DTCContext->EventId, lFreezeFrameSlot, lRequiredUpdates);
      if ((DTCContext->QualifyDTC == TRUE)
...
# else
    (void) Dem_FaultMemory_MemObdFreezeFrame_UpdateOBDIIFreezeFrame(DTCContext->EventId, lFreezeFrameSlot, lRequiredUpdates);
# endif
  }
#else
...
/* ****************************************************************************
 % Dem_FaultMemory_MemObdFreezeFrame_TriggerObdOnUdsFreezeFrameUpdate
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
  ...
  )
{
  if ((DTCContext->DtcEffects & DEM_CFG_TRIGGER_TESTFAILEDTHISCYCLE) != 0u)
  {
    ...
    lFreezeFrameSlot = Dem_Mem_FindOBDFreezeFrameSlot(DTCContext, FALSE);                                                        /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
    Dem_FaultMemory_MemObdFreezeFrame_UpdateObdOnUdsFreezeFrame(DTCContext->EventId, lFreezeFrameSlot);
  }
}                                                                                                                               
