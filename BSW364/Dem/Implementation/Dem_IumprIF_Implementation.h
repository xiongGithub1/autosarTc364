...
)
{
  ...
  /* Satisfy MISRA and assert that numerator and denominator are always initialized. */
  *Numerator = 0x00u;                                                                                                            /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  *Denominator = 0x00u;                                                                                                          /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
       ...
       Dem_Cfg_CombinedGroupIterNext(&lCombinedIter))                                                                            /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    if (Dem_Event_TestEventUnavailable(lSubEventId) == FALSE)
    {
      ...
      lRatioIndex = Dem_Event_GetRatioIdx(lSubEventId);
      Dem_Iumpr_Ratio_GetRatio(lRatioIndex, &lNumerator, &lDenominator);                                                         /* SBSW_DEM_POINTER_FORWARD_STACK */
      ...
      else
      {
        ...
        /* If two ratios have a numerator of 0 and a denominator > 0, the ratio with the bigger denominator is worse. */
        if ( (lNumerator == 0x00u)
          && ( (*Numerator) == 0x00u)
        ...
        /* Expand the ratios so that their denominators are the same and their numerators can be compared directly. */
        if ( ( ((uint32) lNumerator) * (*Denominator) ) < ( ((uint32) (*Numerator)) * lDenominator) )
        {
          *Numerator = lNumerator;                                                                                               /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
          *Denominator = lDenominator;                                                                                           /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
        }
        else if ((((uint32)lNumerator) * (*Denominator)) == (((uint32)(*Numerator)) * lDenominator))
        {
          if (*Denominator < lDenominator)
          {
            *Numerator = lNumerator;                                                                                             /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
            *Denominator = lDenominator;                                                                                         /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
          }
