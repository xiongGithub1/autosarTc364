...
# endif
    ...
    {
      Dem_Cfg_MemoryDataSizeType lDataSize = Dem_Cfg_MemoryDataSize[lBlockIndex];

# if (DEM_DEV_RUNTIME_CHECKS == STD_ON)
      if ((Dem_Cfg_MemoryDataSizeType)sizeof(Dem_Cfg_CommitBuffer) < lDataSize)
      {
        Dem_Error_RunTimeCheckFailed(DEM_NVM_IMPLEMENTATION_FILENAME, __LINE__);                                                 /* SBSW_DEM_POINTER_RUNTIME_CHECK */
        lNvmResult = E_NOT_OK;
      }
...
# endif
      {
        Dem_MemCpy((Dem_DataPtrType)&Dem_Cfg_CommitBuffer.mRaw[0],                                                               /* PRQA S 0315, 0602 */ /* MD_MSR_VStdLibCopy, MD_DEM_20.2 */  /* SBSW_DEM_MEMCOPY_MEMORY_DATA */
