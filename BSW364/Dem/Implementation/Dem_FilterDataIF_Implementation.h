  ...
  )
{
  ...
  if (DTCKind == DEM_DTC_KIND_ALL_DTCS)
  {
    ...
    lReturnValue = Dem_FaultMemory_Memory_TranslateDTCOrigin(DTCOrigin, &lTranslatedMemoryId);                                   /* SBSW_DEM_POINTER_FORWARD_STACK */
    ...
    if (lReturnValue == E_OK)
    {
      ...
      if (Dem_FaultMemory_Memory_IsEventDestination(lTranslatedMemoryId) == TRUE)
      {
        lFilterDataPtr->MemoryId = lTranslatedMemoryId;                                                                        /* SBSW_DEM_ARRAY_POINTER_FILTERDATA */
        lFilterDataPtr->Iterator.EventIndex = Dem_MemState_MemoryGetFirstEvent(lTranslatedMemoryId);                           /* SBSW_DEM_ARRAY_POINTER_FILTERDATA */
      }
      else
      {
        lReturnValue = E_NOT_OK;
  ...
  )
{
  ...
  for (lEventId = Dem_Memory_GetFirstEvent(lFilterDataPtr->MemoryId);
       ...
       lEventId++)
  {
    if (Dem_Client_FilterData_TestEventMatchesJ1939DTCFilter(lFilterDataPtr->DTCStatusFilterType, lFilterDataPtr->NodeIndex, lEventId) == TRUE)
    {
