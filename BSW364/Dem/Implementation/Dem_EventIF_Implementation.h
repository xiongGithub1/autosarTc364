 ...
 *
 * \details       Process event's qualification status and permanent storage
 *
 * \param[in,out] EventContext
 *                Event processing context
 *
...
# endif
        ...
        {
          if (Dem_Readiness_Completion_GetReadinessGroupUpdateFlag(Dem_Readiness_Completion_GetFullReadinessGroupUpdateFlag(), Dem_Cfg_EventReadinessGroup(EventId)) == FALSE)
          {
            Dem_Readiness_Completion_SetReadinessGroupUpdateFlag(Dem_Cfg_EventReadinessGroup(EventId));
          }
        }
...
/* >>>> -------------------------------- Enter Critical Section: DiagMonitor */
    ...
    else
    {
      ...
      if ((Dem_GlobalDiagnostics_Readiness_IsReadinessCalculationSupported() == TRUE)
        ...
        && (Dem_Readiness_Completion_TestStaticallyCalculatedCompletionStates(Dem_Cfg_EventReadinessGroup(EventId), DEM_COMPLETION_PIDCONTEXT_PID01_AND_PIDF501) == FALSE))
      {
        Dem_Readiness_Completion_SetReadinessGroupCompleted(FALSE, Dem_Cfg_EventReadinessGroup(EventId));        /* SBSW_DEM_POINTER_FORWARD_GLOBAL */
      }
    }
...
# endif
      if (Dem_GlobalDiagnostics_Readiness_IsReadinessCalculationSupported() == TRUE)
      {
        /* If readiness calculation is enabled by configuration, mark readiness group of event to recalculate readiness state */
        Dem_Readiness_Completion_SetReadinessGroupUpdateFlag(Dem_Cfg_EventReadinessGroup(EventId));
      }
  ...
  )
{
  ...
  if (Dem_Cfg_IsWwhObdSupportedInVariant() == TRUE)
  {
    ...
    {
      Dem_GlobalState_ActivationMode_DecrementQualifiedActiveEventCount(EventId, TRUE);
    }
  }
...
)
{
  ...
  Dem_UdsStatusByteType lOldExternalEventStatus;
  uint8 lEventQualifyStatus;
  Dem_Cfg_StorageTriggerType lEventQualificationTrigger;
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(ArgPtr)                                                                                       /* PRQA S 3112 */ /* MD_DEM_14.2 */

  lEventStatus = Dem_Event_GetUDSStatus(EventId);
  lOldExternalEventStatus = Dem_Event_GetExternalUDSStatus(EventId, lEventStatus);
  lEventQualifyStatus = Dem_Event_GetQualifyStatus(EventId);
  lEventQualificationTrigger = DEM_CFG_TRIGGER_NONE;
  ...
  { /* set confirmed status to qualified */
    ...
    lEventQualificationTrigger = (lEventQualificationTrigger | DEM_CFG_TRIGGER_CONFIRMED);
# if (DEM_CFG_SUPPORT_TYPE1_COMBINATION == STD_ON)
    {
...
# endif
  ...
  { /* set warningIndicatorRequested status to qualified */
    ...
    Dem_Event_SetQualifyStatus(EventId, lEventQualifyStatus);
    lEventQualificationTrigger = (lEventQualificationTrigger | DEM_CFG_TRIGGER_INDICATOR);
# if (DEM_CFG_SUPPORT_TYPE1_COMBINATION == STD_ON)
    {
      uint8 lEventQualificationStatus = ArgPtr->Specific.CycleQualifyDcy_Internal.EventQualificationStatus;
...
# endif
  ...
  Dem_DiagnosticObserver_ProcessDcyOfEventQualified(EventId, lEventQualificationTrigger);
...
# endif
  {
    ...
    if ((DEM_OPERATIONCYCLE_TEST_DCY_QUALIFIED(Dem_OperationCycle_GetObdCycleStates()) == TRUE)
      ...
      || ((Dem_Cfg_IsObdIIExclusivelySupportedInVariant() == TRUE) && (QualifyEvent == TRUE)))
    {
      ...
      if ((Effects & DEM_CFG_TRIGGER_CONFIRMED) != 0u)
      {
        ...
        if (Dem_GlobalDiagnostics_Readiness_IsReadinessCalculationSupported() == TRUE)
        {
          if (Dem_Readiness_Completion_TestStaticallyCalculatedCompletionStates(Dem_Cfg_EventReadinessGroup(EventId), DEM_COMPLETION_PIDCONTEXT_PID01_AND_PIDF501) == FALSE)
          {
            Dem_Readiness_Completion_SetReadinessGroupCompleted(TRUE, Dem_Cfg_EventReadinessGroup(EventId));     /* SBSW_DEM_POINTER_FORWARD_GLOBAL */
          }
        }
      }
