  ...
  )
{
  ...
  if (DTCFormat == DEM_DTC_FORMAT_UDS)
  {
    ...
    {
      lEventId = Dem_Cfg_GetMasterEvent(Dem_Core_DTC_FindEventFromUdsDtc(DTC, TranslatedMemoryId));
    }
  }
  else
  ...
  if (DTCFormat == DEM_DTC_FORMAT_J1939)
  {
    ...
    { /* Dem_Cfg_FindEventFromJ1939Dtc can not be called with '0' */
      lEventId = Dem_Cfg_GetMasterEvent(Dem_Core_DTC_FindEventFromJ1939Dtc(DTC, TranslatedMemoryId));
    }
  }
  else
#endif
  ...
  )
{
  ...
  if (Dem_FaultMemory_Memory_TranslateDTCOrigin(DTCOrigin, &lTranslatedMemoryId) == E_NOT_OK)                                    /* SBSW_DEM_POINTER_FORWARD_STACK */
  {
