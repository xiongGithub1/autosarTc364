  ...
  )
{
       ...
       Dem_Memory_MemEntry_IterExists(&MemEntryIter) == TRUE;)                                                                   /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
      ...
      && (Dem_DTC_TestSuppressionOnDtcRetrieval(lFreezeFrameFilterData.EventId) == FALSE) )                                      /* PRQA S 3415 */ /* MD_DEM_13.5_cf */                                                            
    {
      ...
      if ( (Dem_Cfg_EventObdRelated(lFreezeFrameFilterData.EventId) == TRUE)
        || (lFreezeFrameFilterData.FilterDTCFormat != DEM_FREEZEFRAMEITERATOR_DTCFORMAT_OBD) )
      {
        ...
        if ((Dem_DTCReporting_TestObdHideOccurrences() == FALSE)
          ...
          || (Dem_Cfg_EventObdRelated(lFreezeFrameFilterData.EventId) == FALSE))
        {
          uint8 lRecordNumber;
      ...
      && (Dem_DTC_TestSuppressionOnDtcRetrieval(lEventId) == FALSE) )                                                            /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
    {
        ...
        || (lFreezeFrameFilterData.FilterDTCFormat != DEM_FREEZEFRAMEITERATOR_DTCFORMAT_OBD) )
      {
          ...
          || (Dem_Cfg_EventObdRelated(lEventId) == FALSE) )                                                                      
        {
          ...
          {
            lFreezeFrameFilterData.TimeSeriesRecordNumber = Dem_FreezeFrameIterator_TimeSeriesGetNextValidSRec(lTimeSeriesEntry, lFreezeFrameFilterData.TimeSeriesRecordNumber);            /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
          }

          if (lFreezeFrameFilterData.TimeSeriesRecordNumber <= 0x4fu)
          {
            if (Dem_RecordReader_0x1904_TimeSeriesSRecIsValid(lEventId, lFreezeFrameFilterData.TimeSeriesRecordNumber) == TRUE)
            {
              if (Dem_FreezeFrameIterator_TimeSeriesSRecIsStored(lTimeSeriesEntry, lFreezeFrameFilterData.TimeSeriesRecordNumber) == FALSE)     /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
              {
