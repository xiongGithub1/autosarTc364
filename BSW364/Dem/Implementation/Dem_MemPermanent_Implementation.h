  ...
  )
{
  ...
  for (lEventId = Dem_MemState_MemoryGetFirstEvent(DEM_CFG_MEMORYID_PRIMARY);                                                    /* PRQA S 0771 */ /* MD_DEM_15.4_opt */
       ...
       ++lEventId)
  {
    ...
    if (Dem_DTC_TestEventStoresPermanentDTC(lEventId) == TRUE)
    {
      ...
      {
        if (Dem_MemState_MemoryGetCurrentSize(DEM_CFG_MEMORYID_PERMANENT) >= Dem_MemState_MemoryGetMaxSize(DEM_CFG_MEMORYID_PERMANENT))
        {
...
Dem_MemPermanent_ConfirmedChronologyRestore(void)
{
  ...
  while (lConfirmedChronoIndex != 0u)
  {
    ...
    if (lEventId != DEM_EVENT_INVALID)
    {
      ...
      if ((Dem_MemState_MemoryGetFirstEvent(DEM_CFG_MEMORYID_PERMANENT) > lEventId)
        || (Dem_MemState_MemoryGetLastEvent(DEM_CFG_MEMORYID_PERMANENT) < lEventId)
