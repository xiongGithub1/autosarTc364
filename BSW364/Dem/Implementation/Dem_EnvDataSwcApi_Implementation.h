...
)
{
  ...
  if (Dem_RecordReader_0x1904_TimeSeriesSRecIsValid(EventId, RecordNumber) == FALSE)
  {
    lReturnValue = DEM_NO_SUCH_ELEMENT;
  }
  else
    ...
    do
    {
      ...
      { /* The record currently exists, but without critical section it could age or displace */
        ...
        { /* This call does not currently interrupt a concurrent data update */
          ...
          if (Dem_FreezeFrameIterator_TimeSeriesSRecIsStored(lEntry, RecordNumber) == TRUE)                                      /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
          {
            Dem_ConstSharedDataPtrType lSourceBuffer;
            Dem_Cfg_MemoryIndexType lMemoryIndex;
            ...
            if (RecordNumber >= DEM_RECORDREADER_SNAPSHOTDATARECORD_TIMESERIES_NORMAL_FIRST)
            {
              lSourceBuffer = 
                ...
                RecordNumber - DEM_RECORDREADER_SNAPSHOTDATARECORD_TIMESERIES_NORMAL_FIRST
                ); 
            }
            ...
            else
            {
              lSourceBuffer = 
                ...
                RecordNumber - DEM_RECORDREADER_SNAPSHOTDATARECORD_TIMESERIES_FAST_FIRST
                );    
            }
#else
            else
