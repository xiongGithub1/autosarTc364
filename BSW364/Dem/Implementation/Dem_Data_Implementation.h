...
# endif
  {
    ...
    /* Get supported mask */
    lPid41 = Dem_Readiness_Support_GlobalPid41SupportedMask();
    /* Engine Type */
    if (Dem_GlobalDiagnostics_Readiness_IsMotorTypeCompressionSupported() == TRUE)
    {
      lPid41 |= DEM_READINESS_SUPPORT_COMPRESSION;
    }
...
# if (DEM_CFG_SUPPORT_EVENTAVAILABLE == STD_ON)
        Dem_Readiness_Support_SetReadinessGroupSupportedInPidFormat(TRUE, lReadinessGroup, &lPid41);                                                     /* SBSW_DEM_POINTER_FORWARD_STACK */
# endif 
        if (Dem_Readiness_Completion_TestStaticallyCalculatedCompletionStates(lReadinessGroup, DEM_COMPLETION_PIDCONTEXT_PID41) == FALSE)
        {
          if (Dem_Readiness_Completion_TestReadinessGroupCompletedInPidFormat(lReadinessGroup, lReadinessCompletedMask) == FALSE)
          {
            ...
            {
              Dem_Readiness_Completion_SetReadinessGroupCompletedInPidFormat(TRUE, lReadinessGroup, &lReadinessCompletedMask);                              /* SBSW_DEM_POINTER_FORWARD_STACK */
            }
            else if (Dem_UDSStatus_Test_TNCTOC(lEventStatus) == TRUE)
            {
              Dem_Readiness_Completion_SetReadinessGroupCompletedInPidFormat(FALSE, lReadinessGroup, &lPid41);                                              /* SBSW_DEM_POINTER_FORWARD_STACK */
            }
            else
            {
              /* do nothing */
    ...
    for (lReadinessGroup = 0x01U; lReadinessGroup < DEM_CFG_READINESS_GROUP_SIZE; lReadinessGroup++)
    {
      ...
      {
        Dem_Readiness_Support_SetReadinessGroupSupportedInPidFormat(FALSE, lReadinessGroup, &lPid41);                          /* SBSW_DEM_POINTER_FORWARD_STACK */
      }
    }
    ...
    if (Dem_GlobalDiagnostics_Readiness_IsMotorTypeCompressionSupported() == TRUE)
    {
      lPid41 &= DEM_READINESS_INITIALIZE_PID41COMPLETED_COMPRESSION;
    }
    else
    {
      lPid41 &= DEM_READINESS_INITIALIZE_PID41COMPLETED_SPARK;
    }

    Dem_Readiness_Completion_SetStaticallyCalculatedCompletionStatesPID(&lPid41, DEM_COMPLETION_PIDCONTEXT_PID41);               /* SBSW_DEM_POINTER_FORWARD_STACK */
...
# if (DEM_CFG_SUPPORT_EVENTAVAILABLE == STD_ON)
    ...
    lSupported = FALSE;
# else
    lSupported = (boolean)((((uint32)Dem_Cfg_GetGlobalPID01SupportedAndCompletedMaskByteB() << 16u) & DEM_READINESS_SUPPORT_CMPRCMPT_MASK) != 0u);
# endif
           ...
           lEventId++)
      {
          ...
          )
        {
          ...
          /* If Event Availability is supported, set readiness group to supported
             Otherwise, it is already statically set in Dem_Readiness_Support_GlobalPid41SupportedMask 
          */
#  if (DEM_CFG_SUPPORT_EVENTAVAILABLE == STD_ON)
          lSupported = TRUE;
#  endif
