 ...
 *                Com_WaitingForConfirmation Parameter if the status of transmit request has been accepted.
    ...
    \context      TASK|ISR2
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_TxLLIf_SetTxAttributes(Com_TxPduInfoIterType ComTxPduId, Std_ReturnType status);
#endif
...
/*! \brief            This function returns the result of Tx Pdu Callout.
    ...
    \trace            CREQ-103167
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(boolean, COM_CODE) Com_TxLLIf_GetTxPduCalloutFuncPtr(Com_TxPduInfoIterType ComTxPduId, P2VAR(PduInfoType, AUTOMATIC, AUTOMATIC) PduInfo);
#endif
...
/*! \brief        This function copies the initial meta data to TxBuffer.
    ...
    \context      TASK|ISR2
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_TxLLIf_CpyInitMetaDataToBffr(Com_TxPduInfoIterType ComTxPduId);
#endif
 ...
 *                RxPdus while processing deferred IPdus
    ...
    \context      TASK|ISR2
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxIndication_CacheDeferredEvent(Com_RxPduInfoIterType comRxPduInfoIdx);
#endif
...
/*! \brief        This function caches a deferred gateway description event.
    ...
    \context      TASK|ISR2
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxIndication_CacheDeferredDescriptionGwEvent(Com_RxPduInfoIterType ComPduId);
#endif
...
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(boolean, COM_CODE) Com_TxLLIf_GetTxPduCalloutFuncPtr(Com_TxPduInfoIterType ComTxPduId, P2VAR(PduInfoType, AUTOMATIC, AUTOMATIC) PduInfo)  /* PRQA S 3673 */ /* MD_MSR_Rule8.13 */
{
  ...
  if(Com_IsTxPduCalloutFuncPtrUsedOfTxPduInfo(ComTxPduId))      /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
  {
    ...
    CalloutResult = Com_GetTxPduCalloutFuncPtr(idx) ((PduIdType) ComTxPduId, PduInfo);  /* SBSW_COM_FCT_PTR_CALL_CSL03 */
  }
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_TxLLIf_CpyInitMetaDataToBffr(Com_TxPduInfoIterType ComTxPduId)
{
  ...
  if(Com_IsTxBufferMetaDataUsedOfTxPduInfo(ComTxPduId) && (Com_IsTxPduInitValueMetaDataUsedOfTxPduInfo(ComTxPduId)))    /* COV_COM_CSL03_ISUSED_OPTIONAL_INDIRECTION */ /* InitValueMetaData is required to be configured for pdus with metadata */
  {
    P2VAR(uint8, AUTOMATIC, COM_VAR_NO_INIT) pTxPduBuffer = Com_GetAddrTxBuffer(Com_GetTxBufferMetaDataStartIdxOfTxPduInfo(ComTxPduId));
    P2CONST(uint8, AUTOMATIC, COM_VAR_NO_INIT) pInitValues = Com_GetAddrTxPduInitValue(Com_GetTxPduInitValueMetaDataStartIdxOfTxPduInfo(ComTxPduId));
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxIndication_CacheDeferredEvent(Com_RxPduInfoIterType comRxPduInfoIdx)
{
  ...
  {
    Com_MainFunctionRxStructIdxOfRxPduInfoType mainFunctionRxIdx = Com_GetMainFunctionRxStructIdxOfRxPduInfo(comRxPduInfoIdx);
    Com_RxNextEventCacheEntryIdxOfMainFunctionRxStructType nextEventCacheEntryIdx = Com_GetRxNextEventCacheEntryIdxOfMainFunctionRxStruct(mainFunctionRxIdx);

    if(Com_GetRxDeferredEventCacheIdxOfRxNextEventCacheEntry(nextEventCacheEntryIdx) < Com_GetRxDeferredEventCacheEndIdxOfMainFunctionRxStruct(mainFunctionRxIdx))
    {
      Com_SetRxPduInfoIdxOfRxDeferredEventCache(Com_GetRxDeferredEventCacheIdxOfRxNextEventCacheEntry(nextEventCacheEntryIdx), comRxPduInfoIdx);        /* SBSW_COM_ARRAY_WRITEACCESS */
      Com_IncRxDeferredEventCacheIdxOfRxNextEventCacheEntry(nextEventCacheEntryIdx);    /* SBSW_COM_CSL_VAR_ACCESS */
    }
    /* #20 Otherwise mark the cache as overflowed */
    else
    {
      Com_SetRxDeferredEventCacheIdxOfRxNextEventCacheEntry(nextEventCacheEntryIdx, COM_NO_HANDLERXPDUDEFERREDIDXOFRXPDUINFO);  /* SBSW_COM_CSL_VAR_ACCESS */
    }
  }
}
#endif
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxIndication_CacheDeferredDescriptionGwEvent(Com_RxPduInfoIterType ComPduId)
{
  ...
  if(Com_IsGwTxPduDescriptionInfoUsedOfRxPduInfo(ComPduId) && Com_IsHandleRxDeferredGwDescriptionUsedOfRxPduInfo(ComPduId))     /* COV_COM_CSL03_ISUSED_OPTIONAL_INDIRECTION */
  {
    ...
    {
      if(Com_IsMainFunctionRouteSignalsStructUsedOfRxPduInfo(ComPduId)) /* COV_COM_SILENT_TX */
      {
        Com_MainFunctionRouteSignalsStructIterType mainFunctionRouteSignalsIdx = Com_GetMainFunctionRouteSignalsStructIdxOfRxPduInfo(ComPduId);
        ...
        {
          Com_GwEventCacheIndexIdxOfMainFunctionRouteSignalsStructType gwEventCacheIndexIdx = Com_GetGwEventCacheIndexIdxOfMainFunctionRouteSignalsStruct(mainFunctionRouteSignalsIdx);
          /* #30 If cache is not full */
          if(Com_GetGwEventCacheIdxOfGwEventCacheIndex(gwEventCacheIndexIdx) < Com_GetGwEventCacheEndIdxOfMainFunctionRouteSignalsStruct(mainFunctionRouteSignalsIdx))  /* COV_COM_GWEVENTCACHE */
          {
            /* #40 Store PDU Id in cache, increase gateway cache index */
            Com_SetRxPduInfoIdxOfGwEventCache(Com_GetGwEventCacheIdxOfGwEventCacheIndex(gwEventCacheIndexIdx), ComPduId);       /* SBSW_COM_GWCACHING */
            Com_IncGwEventCacheIdxOfGwEventCacheIndex(gwEventCacheIndexIdx);    /* SBSW_COM_CSL_VAR_ACCESS */
          }
        }
      }
    }
 ...
 *
**********************************************************************************************************************/
FUNC(Std_ReturnType, COM_CODE) Com_TriggerTransmit_Process(Com_TxPduInfoIterType TxPduId, P2VAR(PduInfoType, AUTOMATIC, COM_APPL_VAR) PduInfoPtr)
{
...
#  if  (COM_TXPDUTTCALLOUTFUNCPTRUSEDOFTXPDUINFO == STD_ON)
    ...
    if(Com_IsTxPduTTCalloutFuncPtrUsedOfTxPduInfo(TxPduId))     /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
    {
      ...
      (void) Com_GetTxPduTTCalloutFuncPtr(idx) ((PduIdType) TxPduId, (P2VAR(PduInfoType, AUTOMATIC, AUTOMATIC)) & PduInfo);     /* SBSW_COM_FCT_PTR_CALL_CSL03 */
    }
#  endif

    /* #50 Copy the PDU buffer data to the provided pointer */
    VStdLib_MemCpy_s(PduInfoPtr->SduDataPtr, PduInfoPtr->SduLength, PduInfo.SduDataPtr, PduInfo.SduLength);     /* PRQA S 0315 */ /* MD_MSR_VStdLibCopy */ /* SBSW_COM_VSTDLIB_SAFE */
 ...
 *
**********************************************************************************************************************/
FUNC(void, COM_CODE) Com_TxConfirmation_Process(Com_TxPduInfoIterType TxPduId)
{
 ...
 *
**********************************************************************************************************************/
FUNC(void, COM_CODE) Com_ResetTxTpConnection(Com_TxPduInfoIterType ComTxPduId)
{
...
**********************************************************************************************************************/
FUNC(Std_ReturnType, COM_CODE) Com_TxLLIf_Transmit(Com_TxPduInfoIterType ComTxPduId)    /* PRQA S 1505 */ /* MD_MSR_Rule8.7 */
{
  /* ----- Local Variables ---------------------------------------------- */
 ...
 *
**********************************************************************************************************************/
FUNC(void, COM_CODE) Com_TxLLIf_CancelTransmit(Com_TxPduInfoIterType ComTxPduId)
{
...
#  if (COM_TXACCESSINFODYNSIGUSEDOFTXTPINFO == STD_ON)
    ...
    if(Com_IsTxAccessInfoDynSigUsedOfTxTpInfo(txTpPduInfoIdx))  /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
    {
      /* #40 Initialize the ComIpdu length with the initial dynamic length */
      Com_TxAccessInfoDynSigIdxOfTxTpInfoType txAccessInfoIdx = Com_GetTxAccessInfoDynSigIdxOfTxTpInfo(txTpPduInfoIdx);
#   if (COM_TXSIGINFOUSEDOFTXACCESSINFO == STD_ON)
      if(Com_IsTxSigInfoUsedOfTxAccessInfo(txAccessInfoIdx))    /* COV_COM_ONLY_SIG_PRESENT */
      {
        Com_SetTxSduLength(ComPduId, (Com_TxSduLengthType) (Com_GetStartByteInPduPositionOfTxSigInfo(Com_GetTxSigInfoIdxOfTxAccessInfo(txAccessInfoIdx)) + ((Com_TxSduLengthType) Com_GetDynamicInitialLengthOfTxTpInfo(txTpPduInfoIdx))));     /* SBSW_COM_IDXPARAM_INTERNALFCT_CSL02 */
        Com_SetTxTmpTpPduLength(txTpPduInfoIdx, (Com_TxSduLengthType) (Com_GetStartByteInPduPositionOfTxSigInfo(Com_GetTxSigInfoIdxOfTxAccessInfo(txAccessInfoIdx)) + ((Com_TxSduLengthType) Com_GetDynamicInitialLengthOfTxTpInfo(txTpPduInfoIdx))));  /* SBSW_COM_CSL02 */
      }
#   endif
#   if (COM_TXGRPSIGINFOUSEDOFTXACCESSINFO == STD_ON)
      if(Com_IsTxGrpSigInfoUsedOfTxAccessInfo(txAccessInfoIdx)) /* COV_COM_ONLY_SIGGRP_PRESENT */
      {
        Com_SetTxSduLength(ComPduId, (Com_TxSduLengthType) (Com_GetStartByteInPduPositionOfTxGrpSigInfo(Com_GetTxGrpSigInfoIdxOfTxAccessInfo(txAccessInfoIdx)) + ((Com_TxSduLengthType) Com_GetDynamicInitialLengthOfTxTpInfo(txTpPduInfoIdx))));       /* SBSW_COM_IDXPARAM_INTERNALFCT_CSL02 */
        Com_SetTxTmpTpPduLength(txTpPduInfoIdx, (Com_TxSduLengthType) (Com_GetStartByteInPduPositionOfTxGrpSigInfo(Com_GetTxGrpSigInfoIdxOfTxAccessInfo(txAccessInfoIdx)) + ((Com_TxSduLengthType) Com_GetDynamicInitialLengthOfTxTpInfo(txTpPduInfoIdx))));    /* SBSW_COM_CSL02 */
      }
...
# if (COM_TXBUFFERUSEDOFTXPDUINFO == STD_ON)
    if(Com_IsTxBufferUsedOfTxPduInfo(ComPduId)) /* COV_COM_CSL03_ISUSED_OPTIONAL_INDIRECTION */
    {
...
# if (COM_RXPDUCALLOUTFUNCPTRUSEDOFRXPDUINFO == STD_ON)
    ...
    if(Com_IsRxPduCalloutFuncPtrUsedOfRxPduInfo(rxPduId))       /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
    {
      ...
      retVal = Com_GetRxPduCalloutFuncPtr(idx) ((PduIdType) rxPduId, (P2CONST(PduInfoType, AUTOMATIC, COM_APPL_DATA)) PduInfoPtr);      /* SBSW_COM_FCT_PTR_CALL_CSL03 */
    }

    /* #30 If the callout result is positive or not configured */
    if(retVal == TRUE)
# endif
    {
...
# endif
      /* #40 Call Com_RxIndication_ProcessPdu to process the passed Rx ComIPdu */
 ...
 *
**********************************************************************************************************************/
FUNC(void, COM_CODE) Com_TpRxIndication_Processing(Com_RxPduInfoIterType rxPduId, Std_ReturnType Result)
{
  ...
  switch (Com_GetRxTpConnectionState(rxTpPduInfoIdx))
  {
    ...
    case COM_WAITING_FOR_INDICATION_RXTPCONNECTIONSTATE:
    {
      ...
      else
      {
        ...
        if(Com_IsRxTpBufferUsedOfRxTpInfo(rxTpPduInfoIdx))      /* COV_COM_CSL03_ISUSED_OPTIONAL_INDIRECTION */
        {
...
# if (COM_RXTPBUFFERMETADATAUSEDOFRXTPINFO == STD_ON)
          ...
          rxPduInfo.SduLength += Com_GetMetaDataLengthOfRxPduInfo(rxPduId);
# endif
...
# if (COM_EXISTS_DEFERRED_SIGNALPROCESSINGOFRXPDUINFO == STD_ON)
          if(Com_IsHandleRxPduDeferredUsedOfRxPduInfo(rxPduId)) /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
          {
            ...
            resetRequired = !Com_RxIndication_Processing(rxPduId, &rxPduInfo);  /* SBSW_COM_FCTCALL_WITH_P2CONST */
          }
          else
# endif
            /* #60 Otherwise initiate processing and reset TP machine afterwards. */
          {
            (void) Com_RxIndication_Processing(rxPduId, &rxPduInfo);    /* SBSW_COM_FCTCALL_WITH_P2CONST */
            resetRequired = TRUE;
          }
        }
      }
 ...
 *
**********************************************************************************************************************/
FUNC(void, COM_CODE) Com_ResetRxTpConnection(Com_RxPduInfoIterType rxPduId)
{
  Com_RxTpInfoIdxOfRxPduInfoType rxTpPduInfoIdx = Com_GetRxTpInfoIdxOfRxPduInfo(rxPduId);
# if (COM_EXISTS_DEFERRED_SIGNALPROCESSINGOFRXPDUINFO == STD_ON)
  if(Com_IsHandleRxPduDeferredUsedOfRxPduInfo(rxPduId)) /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
 ...
 *
**********************************************************************************************************************/
FUNC(BufReq_ReturnType, COM_CODE) Com_CopyRxData_Processing(Com_RxPduInfoIterType rxPduId, CONSTP2VAR(PduInfoType, AUTOMATIC, COM_APPL_DATA) PduInfoPointer, P2VAR(PduLengthType, AUTOMATIC, COM_APPL_VAR) RxBufferSizePtr)     /* PRQA S 3673 */ /* MD_MSR_Rule8.13 */
{
  /* ----- Local Variables ---------------------------------------------- */
  BufReq_ReturnType bufReq_status = BUFREQ_E_NOT_OK;    /* PRQA S 2981 */ /* MD_MSR_RetVal */
  /* ----- Implementation ----------------------------------------------- */
  /* #10 If I-PDU is active */
  if(Com_IsRxPduGrpActive(rxPduId))     /* \trace SPEC-2736846 */
  {
    if(Com_IsRxTpInfoUsedOfRxPduInfo(rxPduId))  /* COV_COM_CSL03_ISUSED_OPTIONAL_INDIRECTION */ /* api pattern ensures that pdu is tp, so RxTpInfo is always present */
    {
      Com_RxTpInfoIdxOfRxPduInfoType rxTpPduInfoIdx = Com_GetRxTpInfoIdxOfRxPduInfo(rxPduId);
      ...
      switch (Com_GetRxTpConnectionState(rxTpPduInfoIdx))
      {
        ...
        case COM_COPYING_RXTPCONNECTIONSTATE:
        {
          ...
          if(PduInfoPointer->SduLength <= (Com_GetRxTpSduLength(rxTpPduInfoIdx) - Com_GetRxTpWrittenBytesCounter(rxTpPduInfoIdx)))
          {
            ...
            {
              if((Com_GetRxTpBufferStartIdxOfRxTpInfo(rxTpPduInfoIdx) + Com_GetRxTpWrittenBytesCounter(rxTpPduInfoIdx)) < Com_GetRxTpBufferEndIdxOfRxTpInfo(rxTpPduInfoIdx)) /* COV_COM_RX_TP_BUFFER */
              {
...
# if (COM_RXTPBUFFERMETADATAUSEDOFRXTPINFO == STD_ON)
                ...
                *RxBufferSizePtr -= Com_GetMetaDataLengthOfRxPduInfo(rxPduId);  /* SBSW_COM_PTR_API */
# endif
