...
/*! \brief    This function exits the tx deadline monitoring exclusive area.
    \details  -
    \pre      -
    \param[in]    idx    index of the indirection from the main function tx to the ISRLock counter
    \trace    DSGN-COM_ISRTHRESHOLD_CONCEPT
    \context  TASK
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_TxDlMon_ExitExclusiveArea(Com_TxProcessingISRLockCounterIdxOfMainFunctionTxStructType idx);
...
/*! \brief    This function checks if the ISR Lock must be opened
    ...
    \context  TASK
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_TxDlMon_ThresholdCheck(Com_TxProcessingISRLockCounterIdxOfMainFunctionTxStructType idx);
#endif
...
/*! \brief    This function exits the rx deadline monitoring exclusive area.
    ...
    \context  TASK
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxDlMon_ExitExclusiveArea(Com_RxMainFunctionProcessingISRLockCounterIdxOfMainFunctionRxStructType idx);
...
/*! \brief    This function checks if the ISR Lock must be opened
    ...
    \context  TASK
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxDlMon_ThresholdCheck(Com_RxMainFunctionProcessingISRLockCounterIdxOfMainFunctionRxStructType idx);
#endif
...
/*! \brief        This function calls the related timeout notification functions.
    ...
    \context      TASK
**********************************************************************************************************************/
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxDlMon_CallTimeOutNotifications(Com_RxMainFunctionProcessingISRLockCounterIdxOfMainFunctionRxStructType idx, Com_SizeOfRxTOutInfoType idxRxTOutInfo);
#endif
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_TxDlMon_ExitExclusiveArea(Com_TxProcessingISRLockCounterIdxOfMainFunctionTxStructType idx)
{
  ...
  Com_SetTxProcessingISRLockCounter(idx, Com_GetTxProcessingISRLockThreshold());        /* SBSW_COM_CSL_VAR_ACCESS */
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_TxDlMon_ThresholdCheck(Com_TxProcessingISRLockCounterIdxOfMainFunctionTxStructType idx)
{
  if(Com_GetTxProcessingISRLockCounter(idx) > 0u)       /* COV_COM_THRESHOLD_TX */
  {
    Com_DecTxProcessingISRLockCounter(idx);     /* SBSW_COM_CSL_VAR_ACCESS */
  }
  else
  {
    Com_TxDlMon_ExitExclusiveArea(Com_GetTxProcessingISRLockCounterIdxOfMainFunctionTxStruct(idx));
    Com_TxDlMon_EnterExclusiveArea();
  }
}
#endif
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxDlMon_ThresholdCheck(Com_RxMainFunctionProcessingISRLockCounterIdxOfMainFunctionRxStructType idx)
{
  ...
  {
    Com_RxDlMon_ExitExclusiveArea(idx);
    Com_RxDlMon_EnterExclusiveArea();
  }
}
#endif
...
COM_LOCAL_INLINE FUNC(void, COM_CODE) Com_RxDlMon_SignalTimeoutAction(Com_SizeOfRxTOutInfoType idxRxTOutInfo)
{
  ...
  Com_RxAccessInfoReplaceSigIndEndIdxOfRxTOutInfoType idxRxAccessInfoReplaceSigInd = Com_GetRxAccessInfoReplaceSigIndStartIdxOfRxTOutInfo(idxRxTOutInfo);       /* PRQA S 2842, 2843 */ /* MD_Com_2842, MD_Com_2843 */ /* Index idxRxTOutInfo originates from Com_RxDlMon_MainFunctionRx(). It is retrieved via an optional indirection by iterating from start to end index, so the optional indirection is always valid */
...
# if ((COM_RXDATATIMEOUTSUBSTITUTIONVALUEUSEDOFRXACCESSINFO == STD_ON) && (COM_BUFFERUSEDOFRXACCESSINFO == STD_ON))
    ...
    {
      Com_Signal_SetRxSigBufferValue(idxRxAccessInfo, FALSE, Com_GetBufferIdxOfRxAccessInfo(idxRxAccessInfo), FALSE, Com_GetRxDataTimeoutSubstitutionValueIdxOfRxAccessInfo(idxRxAccessInfo));
    }
# endif
...
# if ((COM_RXDATATIMEOUTSUBSTITUTIONVALUEUSEDOFRXACCESSINFO == STD_ON) && (COM_SHDBUFFERUSEDOFRXACCESSINFO == STD_ON))
    ...
    {
      Com_Signal_SetRxSigBufferValue(idxRxAccessInfo, TRUE, Com_GetShdBufferIdxOfRxAccessInfo(idxRxAccessInfo), FALSE, Com_GetRxDataTimeoutSubstitutionValueIdxOfRxAccessInfo(idxRxAccessInfo));
    }
# endif
...
FUNC(void, COM_CODE) Com_TxDlMon_MainFunctionTx(Com_MainFunctionTxStructIterType mainFunctionId)
{
  ...
  Com_TxPduInfoIdxOfTxTOutInfoType idxTxPduInfo;
  Com_TxTOutInfoOfMainFunctionTxIndIterType txTOutInfoIndIdx = Com_GetTxTOutInfoOfMainFunctionTxIndStartIdxOfMainFunctionTxStruct(mainFunctionId);
  ...
  /* #10 Iterate over all tx timeout infos referenced from this main function */
  for(; txTOutInfoIndIdx < Com_GetTxTOutInfoOfMainFunctionTxIndEndIdxOfMainFunctionTxStruct(mainFunctionId); txTOutInfoIndIdx++)
  {
    idxTxTOutInfo = Com_GetTxTOutInfoOfMainFunctionTxInd(txTOutInfoIndIdx);
    idxTxPduInfo = Com_GetTxPduInfoIdxOfTxTOutInfo(idxTxTOutInfo);
...
# if ((COM_TXTPINFOUSEDOFTXPDUINFO == STD_ON) && (COM_CANCELLATIONSUPPORTOFTXPDUINFO == STD_ON))
          ...
          Com_TxLLIf_CancelTransmit(idxTxPduInfo);
# endif

# if (COM_CBKTXTOUTFUNCPTRINDUSEDOFTXTOUTINFO == STD_ON)
          Com_TxDlMon_ExitExclusiveArea(txprocessingisrlockcounterIdx);
...
# endif
    ...
    Com_TxDlMon_ThresholdCheck(txprocessingisrlockcounterIdx);
...
FUNC(void, COM_CODE) Com_TxDlMon_SwitchIpduTxMode(Com_TxPduInfoIterType TxPduId)
{
  ...
  if(Com_IsTxTOutInfoUsedOfTxPduInfo(TxPduId))
  {
    ...
    {
      Com_TxModeInfoIdxOfTxPduInfoType idxTxModeInfo = Com_GetTxModeInfoIdxOfTxPduInfo(TxPduId);
...
# if ((COM_PERIODICOFTXMODETRUE == STD_ON) || (COM_PERIODICOFTXMODEFALSE == STD_ON))
      hasCyclicOrDirectTxModePart = (boolean) Com_TxModeHdlr_IsPeriodicTxMode(idxTxModeInfo);   /* PRQA S 2986 */ /* MD_Com_2985_2986 */
# endif
# if ((COM_DIRECTOFTXMODETRUE == STD_ON) || (COM_DIRECTOFTXMODEFALSE == STD_ON))
      {
        boolean isDirectTxMode = (boolean) Com_TxModeHdlr_IsDirectTxMode(idxTxModeInfo);
#  if ((COM_PERIODICOFTXMODETRUE == STD_ON) || (COM_PERIODICOFTXMODEFALSE == STD_ON))
...
FUNC(void, COM_CODE) Com_GwTout_HandleCounter(Com_MainFunctionTxStructIterType mainFunctionId)
{
  ...
  Com_GwTimeoutInfoOfMainFunctionTxIndIterType gwTimeoutInfoIndIdx = Com_GetGwTimeoutInfoOfMainFunctionTxIndStartIdxOfMainFunctionTxStruct(mainFunctionId);   /* PRQA S 2842 */ /* MD_Com_2842 */

  SchM_Enter_Com_COM_EXCLUSIVE_AREA_TX();

  /* #10 Iterate over all gateway timeout infos */
  for(; gwTimeoutInfoIndIdx < Com_GetGwTimeoutInfoOfMainFunctionTxIndEndIdxOfMainFunctionTxStruct(mainFunctionId); gwTimeoutInfoIndIdx++)
  {
    gwToutHndl = Com_GetGwTimeoutInfoOfMainFunctionTxInd(gwTimeoutInfoIndIdx);
...
FUNC(void, COM_CODE) Com_GwTout_InitCounter(Com_TxPduInfoIterType ComTxPduId)
{
  ...
  if(Com_IsGwTimeoutInfoUsedOfTxPduInfo(ComTxPduId))    /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
  {
    ...
    Com_SetGwRoutingTimeoutCounter(Com_GetGwTimeoutInfoIdxOfTxPduInfo(ComTxPduId), 1);  /* SBSW_COM_IDXPARAM_INTERNALFCT_CSL02 */

    /* #30 Stop cyclic transmission */
    Com_TxModeHdlr_StopCyclicTransmission(ComTxPduId);
  }
}
#endif
 ...
 *
**********************************************************************************************************************/
FUNC(void, COM_CODE) Com_GwTout_Event(Com_TxPduInfoIterType ComTxPduId) /* PRQA S 1505 */ /* MD_MSR_Rule8.7 */
{
...
FUNC(void, COM_CODE) Com_RxDlMon_MainFunctionRx(Com_MainFunctionRxStructIterType mainFunctionId)
{
  ...
  Com_RxTOutInfoOfMainFunctionRxIndIterType rxTOutInfoIndIdx = Com_GetRxTOutInfoOfMainFunctionRxIndStartIdxOfMainFunctionRxStruct(mainFunctionId);    /* PRQA S 2842 */ /* MD_Com_2842 */ /* 2842: Index mainFunctionId is checked against SizeOfMainFunctionRxStruct in Com_MainFunctionRxInternal() and does not have invalid indexes. The index is therefore always valid */
  ...
  for(; rxTOutInfoIndIdx < Com_GetRxTOutInfoOfMainFunctionRxIndEndIdxOfMainFunctionRxStruct(mainFunctionId); rxTOutInfoIndIdx++)
  {
    idxRxTOutInfo = Com_GetRxTOutInfoOfMainFunctionRxInd(rxTOutInfoIndIdx);
    ...
    if(Com_LMgt_RxIpduDmActive(idxRxTOutInfo))        /* COV_COM_RX_DM */
    {
...
# if (COM_CBKRXTOUTFUNCPTRINDUSEDOFRXTOUTINFO == STD_ON)
          ...
          Com_RxDlMon_CallTimeOutNotifications(rxMainFunctionProcessingISRLockCounteridx, idxRxTOutInfo);
# endif

        }
      }
    }
    Com_RxDlMon_ThresholdCheck(rxMainFunctionProcessingISRLockCounteridx);
  }
  Com_RxDlMon_ExitExclusiveArea(rxMainFunctionProcessingISRLockCounteridx);
}       /* PRQA S 6080 */ /* MD_MSR_STMIF */
#endif
...
/* Justifications for module-specific MISRA deviations:
    ...
    MD_Com_2843: Misra Rule 1.3, Misra Rule 18.1, Misra Rule 21.18, Misra Dir 4.1
     Reason:     The ComStackLib validates the Index.
     Risk:       No risk.
     Prevention: Qualified use-case of ComStackLib.
